...
if(!$joueurM->is_email_in_bdd($email)){
    ...
    if(!mail($email,
            ...
            . " <a href='http://vps36292.ovh.net/mordu/flottes/index.php?action=generate_mdp&email=".$email."&auth=". $crypt->crypte($email."toutestok".date("Ymd"))."'>http://vps36292.ovh.net/mordu/flottes/index.php?action=generate_mdp&email=".$email."&auth=". $crypt->crypte($email."toutestok".date("Ymd"))."</a>",
            $headers)){
