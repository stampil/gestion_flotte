...
class SortieManager {
    ...
    public function get_range_sortie($date_deb,$date_fin){
        $query="SELECT s.id_sortie, s.titre, s.detail, s.debut, s.fin, s.id_teamspeak, ts.url as url_ts, j.id_joueur, j.handle, t.nom, t.logo as logoTeam
    ...
    public function set_sortie( Sortie $s){
        $query ="INSERT INTO ".MyPDO::DB_FLAG."sortie (id_organisateur,id_teamspeak, titre, detail, debut, fin) VALUE(?,?,?,?,?,?)";
        $this->bdd->query($query,$s->get_id_organisateur(), $s->get_id_teamspeak(), $s->get_titre(), $s->get_detail(), $s->get_debut(), $s->get_fin());
    }
